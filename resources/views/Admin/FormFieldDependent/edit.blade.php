        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.form.field.dependent.edit.save') }}" method="post">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <select class="form-select" name="FieldOptionId" aria-label="Default select example">
                                    ...
                                    <option value="{{$item->id}}"  @if($data->option_id == $item->id) selected @endif>({{$item->field->label}}) - {{$item->value}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <select class="form-select" name="DependentFieldOptionId"
                                    ...
                                    @foreach($fieldoptions as $item)
                                    <option value="{{$item->id}}" @if($data->dependent_option_id == $item->id) selected @endif>({{$item->field->label}}) - {{$item->value}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
